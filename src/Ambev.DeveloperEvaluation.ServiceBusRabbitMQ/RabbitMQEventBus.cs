...
using Ambev.DeveloperEvaluation.ServiceBus.Abstractions;
    ...
    RabbitMQTelemetry rabbitMQTelemetry) : IEventBus, IDisposable, IHostedService
{
    ...
    public Task PublishAsync(IntegrationEvent @event)
    {
        ...
        // messaging specification. https://github.com/open-telemetry/semantic-conventions/blob/main/docs/messaging/messaging-spans.md
        ...
        return _pipeline.Execute(() =>
        {
            ...
            // Depending on Sampling (and whether a listener is registered or not), the activity
            // above may not be created. If it is created, then propagate its context. If it is not
    ...
    {
        // Messaging is async so we don't need to wait for it to complete. On top of this the APIs
        // are blocking, so we need to run this on a background thread.
    ...
    private async Task OnMessageReceived(object sender, BasicDeliverEventArgs eventArgs)
    {
        ...
        // Even on exception we take the message off the queue. in a REAL WORLD app this should be
        // handled with a Dead Letter Exchange (DLX). For more information see: https://www.rabbitmq.com/dlx.html
