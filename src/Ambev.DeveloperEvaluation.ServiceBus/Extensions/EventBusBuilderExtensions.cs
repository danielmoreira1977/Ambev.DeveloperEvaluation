...
namespace Ambev.DeveloperEvaluation.ServiceBus.Extensions;
...
public static class EventBusBuilderExtensions
{
    ...
    {
        // Use keyed services to register multiple handlers for the same event type the consumer can
        // use IKeyedServiceProvider.GetKeyedService<IIntegrationEventHandler>(typeof(T)) to get all
        ...
        {
            // Keep track of all registered event types and their name mapping. We send these event
            // types over the message bus and we don't want to do Type.GetType, so we keep track of
            // the name mapping here.
